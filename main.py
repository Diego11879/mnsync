...
class Watcher:
    DIRECTORY_TO_WATCH = "/home/diego/Music/Sync"
...
# Inicializar el watcher en un hilo separado
def start_watcher():
    w = Watcher()
    w.run()

# Ruta principal para mostrar el estado
@app.route('/')
def index():
...
def status():
    global syncing
    # Aquí puedes agregar lógica para obtener el estado del watcher
    return jsonify({'status': 'sincronizando' if syncing else 'en espera'})
